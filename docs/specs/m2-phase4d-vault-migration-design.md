...
| **`.mcp-config.json` contains the literal Google access token and would sync to Drive** (regression introduced 2026-04-18 by Phase 3b S14 fix: token-in-file replaces `${VAR}` placeholder). | Token leaks to anyone with Drive access to the engagement folder. | **Phase 4d MUST** relocate `.mcp-config.json` outside the vault root (e.g. `~/.ikrs-workspace/runtime/{client-slug}/.mcp-config.json`) OR add it to the Drive-client exclusion list, before the vault path migrates to the Shared Drive. Codex review `2026-04-18-s12-s14-fix-review.md` flagged this; spec amended here to track. |
